...
# Convertir las columnas de "Acuerdo" a numérico
...
# Seleccionar y convertir las columnas restantes a numérico
...
#Convertir Direction a numérico (Direct == 0, Inverse == 1)
...
#Invertir la puntuación 
# Seleccionar las columnas que contienen "IS" y "Direction"
matriz_IS <- data_filtrada %>% select(contains("IS"))
matriz_direction <- data_filtrada %>% select(contains("Direction"))

# Ajustar las puntuaciones de IS (si Direction es 1, - a 100)
...
#Limpiar nombre de las columnas IS_ y Direction_
...
# Lista de grupos para variables con 8 niveles
grupos_todos <- c("UP", "UR", "CP", "CR", "DP", "DR", "NMP", "NMR")

# Lista de grupos para variables con solo 4 niveles
grupos_reducidos <- c("UR", "CR", "DR", "NMR")

# Asignar correctamente 'Group' según la variable
...
# Transformaciones de las variables con 8 grupos
df_reliability <- data_cleaned %>%
  select(identity, starts_with("Reliability")) %>%
...
# Transformaciones de las variables con 4 grupos
df_antroH <- data_cleaned %>%
  select(identity, starts_with("AntroH")) %>%
...
# Crear la base de identidad con todas las combinaciones
base_identity <- expand_grid(
  identity = unique(data_cleaned$identity),
  Group = grupos_todos
)

# Asegurar que cada dataframe tenga todas las combinaciones posibles
...
# Ajustar el modelo mixto para la variable Reliability
...
# Graficar con límites de 1 a 7 en el eje y
...
#Moral Trust
```{r}
# Ajustar el modelo mixto para la variable MoralTrust
...
#Moral Agency
```{r}
# Ajustar el modelo mixto para la variable MoralAgency
...
# Ajustar el modelo mixto para la variable Complejidad
...
# Ajustar el modelo mixto para la variable IS
...
# Obtener los resultados de emmeans
...
# Calcular emmeans por framework
emtab <- emmeans(emmeans_moralIS, ~ framework)

# Transformar a escala original (si aplica)
...
# Graficar sin usar 'type' y con escala de 0 a 100
...
#Antropomorphism basic analysis
```{r}
library(mediation)
library(lavaan)

#Efecto Antropomorfismo Humano
...
```{r}
###############################AntroH################################
###Correlation controlando por framework
...
####Correlation sin contratoar por framework
#Acuerdo AntroH
...
# Run correlations and store results
...
# Run correlations for AntroH and AntroR
